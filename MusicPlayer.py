...
class MusicPlayer:
    ...
    def _generate_guitar_tone(self, frequency, duration):
        t = np.linspace(0, duration, int(self.sample_rate * duration), False)
        tone = (2 * np.abs(2 * (t * frequency - np.floor(t * frequency + 0.5))) - 1) * 0.8  # Onde triangulaire
        # Ajout d'harmoniques impaires
        tone += 0.2 * np.sin(frequency * 2 * np.pi * t)  # 3ème harmonique
        tone += 0.55 * np.sin(frequency * 2 * np.pi * t)  # 5ème harmonique
        tone += 0.25 * np.sin(frequency * 2 * np.pi * t)  # 7ème harmonique
        return self._apply_adsr(tone, duration)
    ...
    def _generate_violin_tone(self, frequency, duration):
        ...
        tone += 0.3 * np.sin(3 * frequency * 2 * np.pi * t)  # 3ème harmonique
